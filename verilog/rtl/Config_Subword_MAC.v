//------------------------------------------------------------------------------
// 文件名: Config_Subword_MAC.v
// 项目: Caravel User Project
// 描述: 可配置子字MAC模块，支持4x4bit, 2x8bit和混合精度模式
//       设计优化用于ASIC流片，包含有符号乘法和饱和逻辑
//------------------------------------------------------------------------------

module Config_Subword_MAC (
    input wire clk,                  // 系统时钟
    ...
    output reg [15:0] accum_out,     // 16位累加结果
    // 调试信号
    ...
    always @(*) begin
        case (mode)
            2'b00: en_slice = 4'b1111;  // 4x4bit模式：所有单元使能
            2'b01: en_slice = 4'b0011;  // 2x8bit模式：前两个单元使能
            2'b10: en_slice = 4'b0111;  // 混合模式：前三个单元使能(2x4bit + 1x8bit)
            default: en_slice = 4'b0000;
        endcase
    end
    ...
    // 第二级累加 - 根据模式选择适当的累加方式
    assign current_sum = (mode == 2'b01) ? sum_stage1_a :  // 2x8bit模式
...
module MAC_wrapper_new (
    ...
    // 从原始控制信号解码新的模式
    reg [1:0] new_mode;      // 新的模式编码: 00:4x4bit, 01:2x8bit, 10:混合模式
    wire clear_acc;
    wire enable;
    ...
    assign enable = alu[4];           // 累加使能
    
    // 实例化新的Config_Subword_MAC模块
    wire overflow_flag;
    wire [15:0] accum_result;
    
    Config_Subword_MAC mac_core (
        .clk(CLK),
        .rst_n(~ASYNCRESET),         // 转换为低电平有效复位
        .mode(new_mode),             // 00:4x4bit, 01:2x8bit, 10:混合模式
        .data_in(a),                 // 输入数据
        .weight(b),                  // 权重数据
        .enable(enable),             // 使能信号
        .clear_acc(clear_acc),       // 清除累加器
        .accum_out(accum_result),    // 16位累加结果
        .overflow_flag(overflow_flag) // 溢出指示
    );
    
    // 输出赋值 - 保持与原始接口兼容
    assign O0 = accum_result;        // 结果输出 (已经饱和处理)
    assign O1 = overflow_flag;       // 溢出标志
    assign O2 = accum_result[8];     // 用于条件模块的位
    assign O3 = new_mode[0];         // 模式位[0]
    assign O4 = new_mode[1];         // 模式位[1]
    assign O5 = alu[2];              // 保留其他模式位
    
    // 完整结果输出
    assign full_result = accum_result;
endmodule